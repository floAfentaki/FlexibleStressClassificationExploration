...
def train(dataset, model, method_name):
    if dataset=='spd':
        ...
        csv_path="./data/spd_statistics_192.csv"
    elif dataset=='wesad':
        csv_path = "./data/90sec_features_n_label_2_SMOTE_True.csv"
    elif dataset=='stress_in_nurses_test':
        csv_path = "./data/stress_in_nurses_test_statistics_192.csv"
    elif dataset=='AffectiveROAD':
        csv_path = "./data/AffectiveROAD_statistics_64.csv"
    elif dataset=='DriveDB':
        csv_path = "./data/DriveDB_statistics_80.csv"
    else:
    ...
    if model=="MLP":
        tuned_parameters = {
            ...
            # 'hidden_layer_sizes': [(10,), (50,), (100,), (10, 50),(10, 100),(50, 10),(50, 50),(50, 100),(100, 10),(100, 50)],
            'activation': ['relu'],  # activation function
            'solver': ['sgd', 'adam'],  # Solver --> weight optimization - 'sgd', 'adam' (adaptive moment estimation--> better for larger datasets)
            'alpha': [1e-5, 1e-4, 1e-3, 0.01],  # L2 penalty (regularization), higher values --> stronger regularization to prevent overfitting
        ...
        tuned_parameters = {
        'num_filters': [32, 64, 128, 256, 512],           # Number of filters in Conv1D
        'kernel_size': [3, 4], 
        'activation': ['relu'],
    ...
    for num_f in feature_sizes:
        ...
        if model=="MLP":
            grid = GridSearchCV(MLPClassifier(max_iter=600), tuned_parameters, refit=True, verbose=3, n_jobs=100)
        elif model=="SVM":
        ...
        # log_file.write(f"Feature Size: {num_f}, Accuracy: {best_accuracy:.4f}, Best Params: {best_params}\n")
        # for params, mean_score, _ in grid.cv_results_['params'], grid.cv_results_['mean_test_score'], grid.cv_results_['std_test_score']:
        #     if mean_score >= best_accuracy - 0.0001:
        #         if 'hidden_layer_sizes' in params and sum(params['hidden_layer_sizes']) < sum(best_params['hidden_layer_sizes']):
        #             best_params = params
        ...
        best_model = grid.best_estimator_
        joblib.dump(best_model, f"{folder_path}/{num_f}.joblib")
...
if __name__=="__main__":
    ...
    # dataset=['wesad','spd']
    # dataset=['wesad']
    # dataset=['spd', 'AffectiveROAD', 'DriveDB', 'stress_in_nurses_test']
    dataset=['DriveDB']
    ...
    # model=['CNN']
    # model=['SVM']
    method_name = ['jmi', 'disr','fisher_score']
