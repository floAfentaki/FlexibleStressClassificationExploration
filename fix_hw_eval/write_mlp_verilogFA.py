...
def writeneuron_module(neuron_id, nweights, bias, actfunc, WIDTH_ACT, WIDTH_W, swidth, rwidth):
    ...
    if actfunc == 'relu':
        ...
        print(f"                  output signed [{rwidth}:0] out);")
    ...
    if actfunc == 'relu':
        ...
        print(f"    assign out = {nsum}[{rwidth}:0];")
...
def write_mlp_verilog(f, inp_width, L0trunc, w_width, int_lst, w_lst):
    ...
    for i, layer in enumerate(WEIGHTS):
        ...
        # SUMWIDTH.append(len(WEIGHTS)*size)
    ...
    for j, layer in enumerate(WEIGHTS):
        for i, nweights in enumerate(layer):
            ...
            if j!=len(WEIGHTS)-1:
                ...
                activation_function=''
            writeneuron_module(f"n_{j}_{i}", nweights, bias, activation_function, WIDTH_ACT[j], WIDTH_W ,swidth, rwidth)
    ...
    for j, layer in enumerate(WEIGHTS):
        ...
        for i in range(len(layer)):
            ...
            if j == 0:
                ...
                print(f"    wire signed [{WIDTH_ACT[j+1]}:0] {prefix}_out;")
