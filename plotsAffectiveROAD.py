...
# Keep only the rows that have Dataset == 'wesad'
data_dt = data_dt[data_dt['Dataset'] == 'AffectiveROAD']
data_svm = data_svm[data_svm['Dataset'] == 'AffectiveROAD']
data_mlp = data_mlp[data_mlp['Dataset'] == 'AffectiveROAD']
...
# fig = plt.figure(figsize=(3.37, 1.35))  # Total width: 3.37in per subplot, 2 rows
fig = plt.figure(figsize=(3.37, 1.2))  # Total width: 3.37in per subplot, 2 rows
# fig, axes = plt.subplots(2, 2, figsize=(3.37, 3))  # Total width: 3.37in per subplot, 2 rows
# Step 6: Reduce space between subplots to maximize figure area
# plt.subplots_adjust(left=0.1, right=0.96, top=0.82, bottom=0.17, wspace=0.2, hspace=0.45)
# plt.subplots_adjust(left=0.1, right=0.96, top=0.81, bottom=0.1, wspace=0.3, hspace=0.5)
plt.subplots_adjust(left=0.1, right=0.96, top=0.97, bottom=0.19, wspace=0.29, hspace=0.49)
    ...
    if len(subset) > 1:
        for i in range(len(subset)):
            for j in range(i + 1, len(subset)):
                distance = np.sqrt((subset.iloc[i]['Power'] - subset.iloc[j]['Power'])**2 + (subset.iloc[i]['Accuracy'] - subset.iloc[j]['Accuracy'])**2)
...
ax0.text(0.75, 0.13, 'd) DT', ha='left', va='center', transform=ax0.transAxes, fontsize=7)
...
for i in range(len(pareto_x)):
    ...
    # print(filtered_data_dt[(filtered_data_dt['Power'] == pareto_x[i]) & (filtered_data_dt['Accuracy'] == pareto_y[i])]['FeatureSelection'].values[0])
    fs_list.append(filtered_data_dt[(filtered_data_dt['Power'] == pareto_x[i]) & (filtered_data_dt['Accuracy'] == pareto_y[i])]['FeatureSelection'].values[0])
...
# ax1.set_yticks(np.round(np.linspace(y_min_svm-4, y_max_svm, 5)))
ax1.set_ylim([y_min_svm-4, y_max_svm+4])  # Set y-axis limits
ax1.tick_params(axis='both', labelsize=7, pad=0.2)
ax1.text(0.2, 0.13, 'e) SVM', ha='left', va='center', transform=ax1.transAxes, fontsize=7)
...
for i in range(len(pareto_x)):
    # print(pareto_x[i], pareto_y[i])
    # print(filtered_data_svm[(filtered_data_svm['Power'] == pareto_x[i]) & (filtered_data_svm['Accuracy'] == pareto_y[i])]['FeatureSelection'].values[0])
...
ax2.text(0.85, 0.13, 'f) MLP', ha='left', va='center', transform=ax2.transAxes, fontsize=7)
...
for i in range(len(pareto_x)):
    # print(pareto_x[i], pareto_y[i])
    # print(filtered_data_mlp[(filtered_data_mlp['Power'] == pareto_x[i]) & (filtered_data_mlp['Accuracy'] == pareto_y[i])]['FeatureSelection'].values[0])
    ...
    # print(filtered_data_mlp[(filtered_data_mlp['Power'] == pareto_x[i]) & (filtered_data_mlp['Accuracy'] == pareto_y[i])]['bitwidth'].values[0])
...
# fig.text(0.5, 0.002, 'Power ($mW$)', ha='center', fontsize=7)
...
#     for feature_selection, marker in marker_styles.items():
#         handle = plt.Line2D([0], [0], marker=marker, color=color, markersize=7, linestyle='None', label=f'{feature_selection} {number}')
...
# for number, color in number_colors.items():
#     handle = plt.Line2D([0], [0], marker='_', color=color, markersize=5, linestyle='None', label=f'{number}')
#     legend_handles.append(handle)
...
# output_pdf_path = os.path.join(output_folder, 'fsPower.pdf')
output_pdf_path = os.path.join(output_folder, 'fsPower_AfectiveROAD.pdf')
