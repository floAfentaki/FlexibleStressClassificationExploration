...
def features(dataset, model, method_name):
    # Load dataset
    if dataset == 'spd':
    ...
    if method_name == 'jmi':
        ...
        print("This feature selection method is not provided yet")
        exit()

    # Get the top 30 feature indices and corresponding names
    top_30_indices = idx[:30]  # Get the first 30 feature indices
    top_30_features = feature_names[top_30_indices]  # Map indices to original feature names
...
def filter(dataset):
    if dataset=='spd':
        ...
        csv_path = "./data/WESAD_simple.csv"
    data = pd.read_csv(csv_path, delimiter=',')

    # Define the prefixes for the required signals and the statistics of interest
    signals = ['temp', 'resp', 'eda', 'emg', 'ecg', 'bvp']
    stats = ['mean', 'sum', 'std', 'min', 'max']

    # Create a list of columns to keep (those that match the signal and the stat)
    columns_to_keep = []
    for signal in signals:
        for stat in stats:
            matching_columns = [col for col in data.columns if col.startswith(signal) and stat in col]
            columns_to_keep.extend(matching_columns)

    # Add the 'label' column if it exists in the original data
...
def train(dataset, model, method_name):
    ...
    folder_path = f'./{model}/{dataset}/{method_name}/float_models_simple'
    ...
    print(f"Selected Features: {idx[:30]}\n")
    selected_feature_names = [data.columns[i] for i in idx[:30]]
