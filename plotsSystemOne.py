...
ours_all_area = [52833602.93/ 1e8, 14297829.44/ 1e8, 417965572.9/ 1e8, 148764773/ 1e8]
ours_all_acc = [100, 96,100, 96]
ours_dataframe=pd.DataFrame(np.array([ours_all_acc, ours_all_area]).T, columns=['Accuracy', 'Total_Area'])
ax0.scatter(
    pareto_dt_areas, 
...
pareto_x, pareto_y = pareto_frontier_2d(ours_dataframe['Total_Area'], ours_dataframe['Accuracy'], maxX=False, maxY=True)
...
# Combine DT and MLP datasets
combined_data = pd.concat(
    [filtered_data_dt.assign(Classifier='DT'), filtered_data_mlp.assign(Classifier='MLP')],
    ignore_index=True
)

# Initialize figure
fig, ax = plt.subplots(figsize=(3.37, 2))
plt.subplots_adjust(left=0.1, right=0.95, top=0.82, bottom=0.15, wspace=0.25, hspace=0.25)

# Define color for classifiers
classifier_colors = {'DT': sns.color_palette("muted", 3)[0], 'MLP': sns.color_palette("muted", 3)[1]}
    # c=sns.color_palette("muted", 1)[0],  # Muted orange from seaborn palette
# Plot combined data
for classifier, group_data in combined_data.groupby('Classifier'):
    for bitwidth, marker in marker_styles.items():
        subset = group_data[group_data['bitwidth'] == bitwidth]
        colors = [classifier_colors[classifier] for _ in subset['bitwidth']]
        ax.scatter(
            subset['Total_Area'],
            subset['Accuracy'],
            c=colors,
            ...
            s=15,
            alpha=0.8,
            label=f'{classifier} {bitwidth}-bits'
        )

# Add pareto points from our_dataframe
pareto_x, pareto_y = pareto_frontier_2d(ours_dataframe['Total_Area'], ours_dataframe['Accuracy'], maxX=False, maxY=True)
ax.scatter(
    ours_dataframe['Total_Area'],
    ours_dataframe['Accuracy'],
    c=sns.color_palette("pastel", 10)[8],
    marker='^',
    s=40,
    edgecolors=sns.color_palette("muted", 4)[3],  # Gray edge color
    linewidths=0.5      # Thin edge line
)
# ax.plot(pareto_x, pareto_y, linestyle='-', color='gray', lw=2, alpha=0.8)
ax.plot(pareto_dt_areas, pareto_dt_accuracies, linestyle='-', color='gray', lw=2, alpha=0.8)
for i, (x, y) in enumerate(zip(ours_dataframe['Total_Area'], ours_dataframe['Accuracy'])):
    if i==0:
        ax.text(
            x - 1,  # Slight offset for better visibility
            y + 3,
            f"Ours\n DT {i}",
            fontsize=6,
            ha='center',
        ...
        ax.text(
            x+1.3,  # Slight offset for better visibility
            y,
            f"Ours\nMLP {i%2}",
            fontsize=6,
            ha='center',
...
x_min, x_max = combined_data['Total_Area'].min(), combined_data['Total_Area'].max()
y_min, y_max = combined_data['Accuracy'].min(), 100
ax.set_xticks(np.round(np.linspace(x_min, x_max, 3), 4))
ax.set_yticks(np.round(np.linspace(y_min, 100, 5)))
ax.tick_params(axis='both', labelsize=7, pad=0.2)
# ax.set_xlabel('Area(cm^2)', fontsize=7)
...
#     for bitwidth, marker in marker_styles.items():
#         handle = plt.Line2D([0], [0], marker=marker, color=color, markersize=7, linestyle='None', label=f'{classifier} {bitwidth}-bits')
#         legend_handles.append(handle)

ax.set_xticks([], minor=True)  # Remove minor ticks
ax.set_yticks([], minor=True)  # Remove minor ticks
# ax.tick_params(axis='both', which='both', bottom=False, top=False, left=False, right=False)
# ax.tick_params(axis='both', labelsize=7, pad=0.0)
# Adjust the tick label padding to bring labels closer to the axes
# ax.tick_params(axis='x', which='both', pad=2)  # Adjust x-axis padding
# ax.tick_params(axis='y', which='both', pad=2)  # Adjust y-axis padding
ax.set_ylim(y_min - 5, y_max + 7)
ax.set_xlim(x_min - 2, x_max+3)
# ax.set_xlabel('Area(cm^2)', fontsize=7)
# ax.set_ylabel('Accuracy', fontsize=7)
# ax0.tick_params(axis='both', labelsize=7, pad=0.2)
fig.text(0.005, 0.5, 'Accuracy', va='center', rotation='vertical', fontsize=7)
fig.text(0.5, 0.03, 'Area(cm^2)', ha='center', fontsize=7)


# Add legend (excluding Pareto points)
legend_handles = []
for bitwidth, marker in marker_styles.items():
    for classifier, color in classifier_colors.items():
        handle = plt.Line2D([0], [0], marker=marker, color=color, markersize=7, linestyle='None', label=f'{classifier} {bitwidth}-bits')
...
# Save the figure
output_pdf_path_combined = os.path.join(output_folder, 'sota_combined_quant.pdf')
plt.savefig(output_pdf_path_combined, format='pdf')
# plt.show()
